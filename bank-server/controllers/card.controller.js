...
export const cardRequest = async (req, res, next) => {
  try {
    const requestedCard = await cardService.cardRequest(req.user._id, req.body);
...
export const approveCardRequest = async (req, res, next) => {
  try {
    const { id } = req.params;
    const cardApproved = await cardService.approveCardRequest(id, req.user._id);
...
// Reject card request
export const rejectCardRequest = async (req, res, next) => {
  try {
    const { id } = req.params;
    const cardRejected = await cardService.rejectCardRequest(id);
...
export const blockCard = async (req, res, next) => {
  try {
    const { id } = req.params;
    const blockedCard = await cardService.blockCard(id, req.user._id);
...
export const unblockCard = async (req, res, next) => {
  try {
    const { id } = req.params;
    const unblockedCard = await cardService.unblockCard(id, req.user);
...
export const cancelCard = async (req, res, next) => {
  try {
    const { id } = req.params;
    const cancelledCard = await cardService.cancelCard(id, req.user);
