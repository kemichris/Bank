...
export const approveCardRequest = async (cardId) => {
  ...
  try {
    ...
    if (card.status !== "pending") {
      throw new ApiError(400, "Only pending card requests can be approved.");
    }
...
export const blockCard = async (cardId, currentUser) => {
  ...
  if (isCardExpired(card.expiryMonth, card.expiryYear)) {
    throw new ApiError(400, "Expired cards cannot be blocked.");
  }
...
export const unblockCard = async (cardId, currentUser) => {
  ...
  if (!blockedByUser) {
    throw new ApiError(404, "Unable to determine who blocked this card.");
  }
  ...
  const isCardOwner = card.owner.toString() === currentUser.id;

  // If an administrator blocked it,
  // only another administrator may unblock it.
  if (blockerRole !== "user" && !isAdmin) {
    throw new ApiError(
      403,
  ...
  if (blockerRole === "user" && !isAdmin && !isCardOwner) {
    throw new ApiError(403, "You are not authorized to unblock this card.");
  }
...
export const cancelCard = async (cardId) => {
  ...
  if (card.status === "pending") {
    throw new ApiError(400, "Pending card requests cannot be cancelled.");
  }

  if (card.status === "cancelled") {
    throw new ApiError(400, "Card has already been cancelled.");
  }
...
export const getCard = async (cardId) => {
  const card = await Card.find({ cardId }).populate(
    "owner",
    "firstName lastName",
  );
